...
export type Memory={jobId:string;accountNumber:number;stage:string;completed:string[];remaining:string[];failedActions:string[];lastAction?:string;lastError?:string;lastUrl?:string;currentStep?:number;loopCount:number;updatedAt:string};
export type PageElement={id:string;tag:string;role?:string;type?:string;name?:string;label?:string;text?:string;placeholder?:string;value?:string;checked?:boolean;disabled?:boolean;required?:boolean;minLength?:number;maxLength?:number;pattern?:string;autocomplete?:string;href?:string};
