...
export async function runJob(o:{jobId:string;url:string;goal:string;count:number;gmailFile:string;showBrowser:boolean;startAccount?:number;onStatus:(s:string,a?:number)=>void;shouldStop:()=>boolean;shouldPause:()=>boolean}){
 ...
 for(let i=o.startAccount||1;i<=o.count;i++){
  ...
  try{
   ...
   let memory:Memory=getMemory(o.jobId,i)||{jobId:o.jobId,accountNumber:i,stage:plan[0]?.title||"Inspect page",completed:[],remaining:plan.map(x=>x.title),failedActions:[],loopCount:0,updatedAt:new Date().toISOString()};const watchdog=new Watchdog();
   for(let step=0;step<Number(process.env.MAX_STEPS||100);step++){
    ...
    if(/captcha|anti[- ]bot|identity verification|security check|two[- ]step|2fa/i.test(snap.text)){result.status="paused";result.error="Manual verification required";await shot(page,screenshots,"manual-verification");addEvent(o.jobId,i,"manual","Manual security verification required");break}
    ...
    if(/user(name)?.{0,25}(taken|unavailable|already exists|not available)/i.test(`${snap.text} ${snap.errors.join(" ")}`)){
      ...
      if(userEl){await page.locator(`[data-agent-id="${userEl.id}"]`).fill(identity.username);continue}
    }
    ...
    if(/verification code|verification email|enter code|one[- ]time password|\botp\b/i.test(snap.text)){
      ...
      if(otp){const codeEl=snap.elements.find(e=>/code|otp|verification/i.test(`${e.label} ${e.name} ${e.placeholder}`));if(codeEl){await page.locator(`[data-agent-id="${codeEl.id}"]`).fill(otp);addEvent(o.jobId,i,"verification","Filled verification code from Gmail");continue}}
    }
    ...
    if(action.type==="done"){result.status="success";memory.completed.push(memory.stage);memory.remaining=[];break}
    if(action.type==="pause"){result.status="paused";result.error=action.reason||"Agent requested manual review";await shot(page,screenshots,"paused");break}
    ...
    memory.updatedAt=new Date().toISOString();saveMemory(memory);saveIdentity(o.jobId,i,identity);if(step%5===0)await shot(page,screenshots,`step-${step}`);
   }
